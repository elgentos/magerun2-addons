...
abstract class AbstractCrackCommand extends AbstractMagentoCommand
{
    ...
    protected \Magento\User\Model\ResourceModel\User\Collection $adminUserCollection;
    protected \Magento\Store\Model\ResourceModel\Store\Collection $storesCollection;

    protected DirectoryList $directoryList;
    private \Magento\Framework\Filesystem\Io\FileFactory $fileFactory;

    public function inject(
        \Magento\Framework\Encryption\EncryptorInterface $encryptor,
        \Magento\User\Model\ResourceModel\User\Collection $adminUserCollection,
        \Magento\Store\Model\ResourceModel\Store\Collection $storesCollection,
        DirectoryList $directoryList,
        \Magento\Framework\Filesystem\Io\FileFactory $fileFactory
    ) {
...
HTML;
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $results = $engine->crack($credentials, $output);
    ...
    protected function generateSpecialWordlist()
    {
        ...
        $stores = $this->storesCollection->getItems();
        $generator = new WordlistGenerator();
        $generator->setAdmins($admins);
        $generator->setStores($stores);
        $words = $generator->generate();

        $dir = $this->directoryList->getPath(DirectoryList::TMP) . DIRECTORY_SEPARATOR . 'password-cracker' . DIRECTORY_SEPARATOR;

        $id = uniqid('run-', true);
        $this->specialPath = $dir . sprintf('special-words-%s.txt', $id);
        @mkdir($dir, recursive: true);

        file_put_contents($this->specialPath, '');
        foreach ($words as $word) {
            file_put_contents($this->specialPath, $word . PHP_EOL, FILE_APPEND);
        }
